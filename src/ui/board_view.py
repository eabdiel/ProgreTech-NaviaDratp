...
# BoardView renders the battlefield and surrounding UI panels. It should display state only; game rules belong in src/engine.
class BoardView:
    ...
    def _keep_card_rects(self, owner):
        y = 820 if owner == 1 else 34
        start_x = self.origin_x
        width = 72
        gap = 10
        return [(i, pygame.Rect(start_x + i * (width + gap), y, width, 84)) for i in range(7)]

    # Converts screen pixels into board coordinates so mouse clicks can be passed into engine-safe row/column logic.
    ...
    def _draw_right_panels(self, screen) -> None:
        ...
        overview = pygame.Rect(1260, 296, 284, 390)
    ...
    def _draw_keep_cards(self, screen, game_state) -> None:
        for owner in [2, 1]:
            label_y = 12 if owner == 2 else 784
            y = 34 if owner == 2 else 820
...
# ---- Beta v1.3 stable BoardView helper overrides ----
def _beta_draw_wrapped(self, screen, text, x, y, max_width, color) -> None:
...
def _beta_draw_selected_piece_panel(self, screen, game_state) -> None:
    ...
    piece = None
    selected_square = getattr(self, "selected_info_square", None)
    if selected_square:
        piece = game_state.board.get_piece(*selected_square)
...
BoardView._draw_wrapped = _beta_draw_wrapped
BoardView._draw_selected_piece_panel = _beta_draw_selected_piece_panel
